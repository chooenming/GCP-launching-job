...
class InstanceGroupCreator:
    ...
    def launch_instance_group(self) -> List[int]:
        ...
        instance_ids = self._get_instancce_ids(self.name, self.node_count)
        return instance_ids
    ...
    def _create_instance_group(self) -> compute_v1.InstanceGroupManager:
        ...
        wait_for_extended_operation(operation, "manage instance group creation")

        self.logger.info("Instance group created successfully...")
        return instance_group_manager_client.get(
            project=self.project_id,
            zone=self.zone,
            instance_group_manager=self.name
        )
    
    def _get_instancce_ids(self, name: str, node_count: int) -> List[int]:
        instance_ids = set()
        trial = 0
        max_trials = 10
        base_sleep_time = 1.5
        while trial <= max_trials:
            self.logger.info(f"Waiting for instance ({trial=})...")
            pager = self.list_instances_in_group(name)
            for instance in pager:
                if instance.id:
                    self.logger.info(f"Instance {instance.id} is ready")
