...
class InstanceTemplateCreator:
    # scopes: used to determine what permissions an identity has on specific resource
    # labels: used to investigate money spent on specific resources
    def __init__(self, 
                 scopes: list[str], 
                 network: str,
                 subnetwork: str,
    ...
    def create_template(self) -> compute_v1.InstanceTemplate:
        ...
        operation = template_client.insert(
            ...
            instance_template_resource=self.template
        )
        wait_for_extended_operation(operation, "instance template creation")

        self.logger.info("Instance template created successfully...")

        return template_client.get(
            project=self.project_id,
            instance_template=self.template_name
        )

    def create_boot_disk(self) -> None:
        ...
        boot_disk.auto_delete = True # after training is complete, dont want to keep any data from this training machine
        boot_disk.boot = True
        boot_disk.device_name = self.boot_disk_config.name

        # attach the properties to the template
        self.template.disks = [boot_disk]
    ...
    def _attach_disks(self) -> None:
        ...
        for disk_name in disk_names:
            disk = compute_v1.AttachedDisk(auto_delete=True, boot=False, mode="READ_ONLY", device_name=disk_name, source=disk_name)
            self.template.properties.disks.append(disk)

        # need to know the name to mount the disks in the startup script
        if len(disk_names) > 0:
    ...
    def _create_network_interface(self) -> None:
        ...
        network_interface.name = "nic0" # default name for the first network interface
    ...
    def _create_machine_configuration(self) -> None:
        ...
        # attach service account to the VM configurations to have some permissions required
        self.template.properties.service_accounts = [
    ...
    def _attach_metadata(self) -> None:
        ...
        for meta_data_name, meta_data_value in self.vm_metadata_config.items():
            self.template.properties.metdata.items.append(compute_v1.Items(key=meta_data_name, value=str(meta_data_value)))
