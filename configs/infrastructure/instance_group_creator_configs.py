...
class InstanceGroupCreatorConfig:
    ...
    instance_template_name: InstanceTemplateCreatorConfig = InstanceTemplateCreatorConfig()
    name: str = SI("${infrastructure.mlflow.experiment_name}-${infrastructure.mlflow.run_name}-${now:%Y-%m-%dT%H:%M:%S}")
    node_count: int = 1
